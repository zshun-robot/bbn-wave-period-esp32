...
void read_and_processIMU_data() {
  ...
  if (t > warmup_time_sec(useMahony)) {
    ...
    freq = estimate_freq(useFrequencyTracker, &arFilter, &kalmANF, &freqDetector, a_noisy, a_no_spikes, delta_t, t);
  ...
  if (isnan(freq) || isnan(freq_adj)) {
    ...
    float ap_wavelength = TrochoidalWave<float>::wavelengthFromAngularFrequency(2.0 * M_PI * freq_adj);
